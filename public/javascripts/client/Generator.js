...
* 
* XXX public/javascripts/server/generate.js
* 
* TODO:
...
var Generator = function () {
	...
	// For meeting max requirements
		...
		var thisTracker = tracker.slice();  // global object `tracker`
	 ...
	 console.log('reached:', reachedCount);
						return null;
					}
					schedIndx += 1;
				}
	...
	*/
	...
	var vacationLimitation = function( indexes ) {
	// Converts an array of indexes into an array of schedules
	// This array just contains the indexes of the actual combos in the main
	// combo array, or null
		var result = [];
		...
		// If the resident had no vacations
		} else {
			result = combosArr;
		}
	...
	var cardioDermLimit;
	var residents;
	gen.generate = function( resids, includeLimit ) {
	...
	// console.log('----- generating from:', resids);
		...
		for ( var resi = 0; resi < residents.length; resi++ ) {//residents.length; resi++ ) {
			...
			var possible 	  = vacationLimitation( resident.possible );
		...
		console.log( 'Done:', elapsed( oldTime ) );
